		...
		do {
			...
			case 1:
				...
				if (vip) {
					...
					boolean selladoResult = ControladorSellos.sellarCarnet(carnet, false);
					if (!selladoResult) {
						System.out.println("Ha ocurrido un error al sellar el carnet");
						break;
					}
				}
				VisitadasDAO.getVisitadasDAO().insertarVisita(peregrino,
						ParadaDAO.getParadaDAO().obtenerParada(actual));
